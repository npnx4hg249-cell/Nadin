...
export function UploadModal({ isOpen, onClose }: UploadModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: () => ingestApi.uploadDataset(file!, name, description || undefined, isPublic),
    onSuccess: () => {
  ...
  return (
    <Modal
      ...
      description="Upload a data file to create a new dataset"
      size="md"
      footer={
        <>
          ...
          >
            {file ? (
              ...
              <div className="text-sm text-gray-400">
                ...
                <p className="text-xs text-gray-600 mt-1">.csv, .xlsx, .xls, .json, .parquet</p>
              </div>
            )}
          </button>
          <input
        ...
        <label className="flex items-center gap-3 cursor-pointer select-none">
          <div className="relative">
            ...
            <div className={cn(
              ...
              isPublic ? 'bg-blue-600 border-blue-600' : 'bg-gray-700 border-gray-600',
            )} />
            <div className={cn(
          ...
          <span className="text-sm text-gray-300">Make dataset public</span>
        </label>
      </div>
    </Modal>
  )
